...
public class RealtimeProvisioningHelperCommand extends AbstractBaseAdminCommand implements Command {
  ...
  @Option(name = "-ingestionRate", required = true, metaVar = "<String>",
      usage = "Avg number of messages per second ingested on any one stream partition (assumed all partitions are uniform)")
  ...
  public String description() {
    return "Given the table config, partitions, retention and a sample completed segment for a realtime table to be setup, "
  ...
  public boolean execute() throws IOException {
    ...
    if (segmentProvided) {
      ...
      memoryEstimator = new MemoryEstimator(tableConfig, new File(_sampleCompletedSegmentDir), _ingestionRate,
          maxUsableHostMemBytes, tableRetentionHours);
    } else {
      ...
      memoryEstimator = new MemoryEstimator(tableConfig, schema, schemaWithMetaData, _numRows, _ingestionRate,
          maxUsableHostMemBytes, tableRetentionHours);
    }
