...
public class ORCRecordExtractorTest extends AbstractRecordExtractorTest {
  ...
  protected void createInputFile() throws IOException {
    TypeDescription schema = TypeDescription.fromString("struct<" + "userID:int," + "firstName:string,"
        + "bids:array<int>," + "cost:double," + "timestamp:bigint,"
  ...
  protected List<Map<String, Object>> getInputRecords() {
    ...
    Map[] simpleStructs = new Map[] { null, createStructInput("structString", "abc", "structLong", 1000L,
        "structDouble", 5.99999), createStructInput("structString", "def", "structLong", 2000L, "structDouble",
            6.99999), createStructInput("structString", "ghi", "structLong", 3000L, "structDouble", 7.99999) };
    ...
    Map[] complexStructs = new Map[] { createStructInput("structString", "abc", "nestedStruct",
        createStructInput("nestedStructInt", 4, "nestedStructLong", 4000L)), createStructInput("structString", "def",
            "nestedStruct",
            createStructInput("nestedStructInt", 5, "nestedStructLong", 5000L)), null, createStructInput("structString",
                "ghi", "nestedStruct", createStructInput("nestedStructInt", 6, "nestedStructLong", 6000L)) };

    // complex list element - each element contains a struct of int and double
    List[] complexLists =
        new List[] { Arrays.asList(createStructInput("complexListInt", 10, "complexListDouble", 100.0),
            createStructInput("complexListInt", 20, "complexListDouble", 200.0)), null, Collections
                .singletonList(createStructInput("complexListInt", 30, "complexListDouble", 300.0)), Arrays.asList(
    ...
    Map[] simpleMaps = new Map[] { createStructInput("key1", 10, "key2", 20), null, createStructInput("key3",
        30), createStructInput("key4", 40, "key5", 50) };

    // complex map with struct values - struct contains double and string
    Map[] complexMap = new Map[] { createStructInput("key1",
        createStructInput("doubleField", 2.0, "stringField", "abc")), null, createStructInput("key1",
            createStructInput("doubleField", 3.0, "stringField", "xyz"), "key2",
            createStructInput("doubleField", 4.0, "stringField", "abc123")), createStructInput("key1",
                createStructInput("doubleField", 3.0, "stringField", "xyz"), "key2",
  ...
  protected Set<String> getSourceFields() {
    return Sets.newHashSet("userID", "firstName", "bids", "cost", "timestamp", "simpleMap", "simpleStruct",
        "complexStruct", "complexList", "complexMap");
  }
