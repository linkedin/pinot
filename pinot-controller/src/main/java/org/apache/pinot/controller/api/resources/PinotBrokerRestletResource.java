...
public class PinotBrokerRestletResource {
  ...
  public List<InstanceInfo> getBrokersForTableV2(
      ...
      @ApiParam(value = "ONLINE|OFFLINE") @QueryParam("state") String state) {
    try {
      List<String> tableNamesWithType = _pinotHelixResourceManager.getExistingTableNamesWithType(tableName,
  ...
  public SuccessResponse toggleQueryRateLimiting(
      @ApiParam(value = "Broker instance name", required = true,
          example = "Broker_my.broker.com_30000") @PathParam("instanceName") String brokerInstanceName,
