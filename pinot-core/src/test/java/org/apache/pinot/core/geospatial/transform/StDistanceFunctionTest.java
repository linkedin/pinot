...
public class StDistanceFunctionTest extends GeoFunctionTest {
  ...
  private void assertDistance(String wkt1, String wkt2, double distance, boolean geom) throws Exception {
    assertDoubleFunction(
        String.format("ST_Distance(%s(%s), %s(%s))", geom ? "ST_GeomFromText" : "ST_GeogFromText", STRING_SV_COLUMN,
            geom ? "ST_GeomFromText" : "ST_GeogFromText", STRING_SV_COLUMN2),
        new double[] { distance },
        Arrays.asList(new Column(STRING_SV_COLUMN, FieldSpec.DataType.STRING, new String[] { wkt1 }),
